...
class BaseMeta(BaseAttack):
    ...
    def __init__(self, args, model=None, nnodes=None, feature_shape=None, lambda_=0.5, attack_structure=True, attack_features=False, undirected=True, device='cpu'):
        ...
        if attack_structure:
            ...
            self.adj_changes = Parameter(torch.HalfTensor(nnodes, nnodes)).to(self.args.device)
            self.adj_changes.data.fill_(0)

        if attack_features:
            assert feature_shape is not None, 'Please give feature_shape='
            self.feature_changes = Parameter(torch.HalfTensor(feature_shape)).to(self.args.device)
    ...
    def self_training_label(self, data, labels, idx_train):
        # Predict the labels of the unlabeled nodes to use them for self-training.
        if self.args.IB == True:
            _, _, _, _, _, output = self.surrogate(data)
        else:
            output = self.surrogate(data)
...
class Metattack(BaseMeta):
    ...
    def __init__(self, args, model, nnodes, feature_shape=None, attack_structure=True, attack_features=False, undirected=True, device='cpu', with_bias=False, lambda_=0.5, train_iters=100, lr=0.1, momentum=0.9):
        ...
        super(Metattack, self).__init__(args, model, nnodes, feature_shape, lambda_, attack_structure, attack_features, undirected, device)
        ...
        for ix, nhid in enumerate(self.hidden_sizes):
            weight = Parameter(torch.HalfTensor(previous_size, nhid).to(device))
            w_velocity = torch.zeros(weight.shape).to(device)
            self.weights.append(weight)
            self.w_velocities.append(w_velocity)

            if self.with_bias:
                bias = Parameter(torch.HalfTensor(nhid).to(device))
        ...
        output_weight = Parameter(torch.HalfTensor(previous_size, self.nclass).to(device))
        output_w_velocity = torch.zeros(output_weight.shape).to(device)
        self.weights.append(output_weight)
        self.w_velocities.append(output_w_velocity)

        if self.with_bias:
            output_bias = Parameter(torch.HalfTensor(self.nclass).to(device))
    ...
    def attack(self, data, ori_features, ori_adj, labels, idx_train, idx_unlabeled, n_perturbations, ll_constraint=False, ll_cutoff=0.004):
        ...
        labels_self_training = self.self_training_label(data, labels, idx_train)
        modified_adj = ori_adj.half()
        modified_features = ori_features.half()

        for i in tqdm(range(n_perturbations), desc="Perturbing graph"):
            if self.attack_structure:
                modified_adj = self.get_modified_adj(ori_adj).half()

            if self.attack_features:
                modified_features = (ori_features + self.feature_changes).half()

            adj_norm = utils.normalize_adj_tensor(modified_adj).half()
        ...
        # np.save('baseline/result/mettack/meta_train_loss.npy', np.array(self.loss_list))
        # np.save('baseline/result/mettack/meta_acc.npy', np.array(self.acc_list))
        # np.save('baseline/result/mettack/meta_attack_loss.npy', np.array(self.attack_loss_list))
